  ...
  return (
    alert?.title && (
      <motion.div
        ...
        <div>
          <p className="text-black-100 text-sm md:text-lg font-semibold">{alert?.title}</p>

          <div className="text-xs md:text-sm font-medium text-grey-500 mt-2">{alert?.message}</div>
        </div>
      </motion.div>
    )
  );
        ...
        <div>
          <div className="flex justify-between items-center">
            <p className="text-black-100 text-xs md:text-sm font-semibold">
              {loadingSetup ? 'Setting up your wallet' : 'Transaction in progress'}
            </p>
          ...
          <div className="text-[10px] md:text-xs font-medium text-grey-500 mt-1">
            Wait for confirmation!. If {"you're"} connected to your mobile wallet, please check your
            phone for a prompt to approve this transaction.
          </div>
        </div>
      </motion.div>
    );
