...
const VaultChanges = ({ amount, generated }: { amount: number; generated: number }) => {
  ...
  const newCollateralRatio = (
    ((Number(user.borrowedAmount) + Number(user.accruedFees)) * 100) /
    collateralWorthInCurrency
  ).toFixed(2);

  const collateralDeposited =
    ((Number(user.depositedCollateral) + Number(amount)) * 100) /
    Number(collateral.liquidationThreshold);

  const liquidationPrice =
    collateralWorthInCurrency / Number(user.depositedCollateral) + Number(amount);
